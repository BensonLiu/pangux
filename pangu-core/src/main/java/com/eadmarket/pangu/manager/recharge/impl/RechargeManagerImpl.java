      ...
      public Boolean doInTransaction(TransactionStatus status) {
        ...
        try {
          int
              updateCount =
              rechargeDao
                  .updateStatus(rechargeId, RechargeStatus.COMPELETED, rechargeDO.getStatus());
          if (updateCount > 0) {
          ...
          LOG.error("failed to finish recharge, id:" + rechargeId + ",outOrderId:" + outOrderId,
                    ex);
        }
        return result;
      }
  ...
  public Long create(RechargeDTO rechargeDTO) throws ManagerException {
    ...
    try {
      ...
      throw new ManagerException(ExceptionCode.SYSTEM_ERROR,
                                 "failed to creat recharge" + rechargeDTO, ex);
    }
  }
