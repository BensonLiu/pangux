...
public class TradeRelatedTimerTask {
   ...
   */
  @Setter
  private Long eadmarketAccountId;
    ...
    Boolean success = adTransactionTemplate.execute(new TransactionCallback<Boolean>() {
      ...
      public Boolean doInTransaction(TransactionStatus status) {
        ...
        try {
          int
              updateCount =
              tradeDao.updateStatus(trade.getId(), trade.getStatus(), TradeStatus.COMPLETED);
          if (updateCount > 0) {
            ...
            advertiseDao.updateAdvertiseById(advertise);
                         ...
                         * 更新广告契约，置为废弃状态
                         */
            advertiseDao.updateContractStatusByAdvertiseId(trade.getPositionId(),
                                                           AdvertiseContractDO.INVALID_STATUS);
            LOG.warn("Trade {} expired, update position {} to ON_SALE", trade.getId(),
                     trade.getPositionId());
          } else {
  ...
  private final class TransferMoneyTransaction implements TransactionCallback<Boolean> {
    ...
    public Boolean doInTransaction(TransactionStatus status) {
      try {
        ...
        int
            rangeDays =
            DateUtils.truncatedCompareTo(transferDate, trade.getLastTransferDate(), Calendar.DATE);
				/*
				 * 还是上述的情况，万一时间程序在开发人员不知道的情况下crash两天，而且交易本来应该在一天前结束的，那么要算出交易之后经历的天数
				 */
        int
            invalidDays =
            DateUtils.truncatedCompareTo(transferDate, trade.getEndDate(), Calendar.DATE);
        if (invalidDays > 0) {
