...
public final class WebsiteAlexaUpdater {
  ...
  /**
   * 1.如果获取到了alexa排名指并且和数据库不同，那么更新数据中的alexa 2.如果获取到了china排名指并且和数据库不同，那么更新数据中的localRank
   */
  private void updateWebsiteRankValueIfNecessary(ProjectDO project) {
    ...
    if (!rankValuePair.success
        || (rankValuePair.alexaRankValue.equals(project.getAlexa()) && rankValuePair.chinaRankValue
        .equals(project.getLocalRank()))
      ...
      while ((currentLine = bufferedReader.readLine()) != null) {
        if (currentLine.contains("<POPULARITY ")) {
          ...
          pair.alexaRankValue = alexaRankValue;
            ...
            .contains("<COUNTRY CODE=" + '"' + "CN" + '"' + " NAME=" + '"' + "China")) {
          currentLine = currentLine.replaceAll("[^0-9]", "").trim();
